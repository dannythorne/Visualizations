...
public class LGA extends PApplet {
...
public void setup() {
  ...
  g_dy = 1;
  g_lu = width/10;
  g_x0 = g_lu*g_dx*(5.f/8.f); //width/2 - g_lu*g_dx;
  g_y0 = g_lu*g_dy*(5.f/8.f); // height/2 - (((float)(nj-1)/2)*g_lu*g_cospio3)*g_dy;
...
class HexGrid {
  ...
  class HexNode {
    ...
    public boolean is_solid() { return (m_v & 0x80) == 0x80;}
  ...
  HexGrid( int ni, int nj) {
    ...
    for( j=0; j<m_nj; j++) {
      ...
      if( rowoff==0) { rowoff=.5f*m_h;} else { rowoff=0;}
    }
    ...
    if( !g_list_all) {
      ...
      {
        // A solid obstacle.
        for( j=floor(.35f*nj); j<floor(.65f*nj); j++) {
          for( i=floor(.5f*ni); i<floor(.5f*ni)+1; i++) {
        ...
        for( i=0; i<m_ni; i+=m_ave_size)
        {
          ...
          for( jj=j; jj<j+m_ave_size; jj++)
          {
            for( ii=i; ii<i+m_ave_size; ii++)
            {
              ...
              vx += ((nodes[ii][jj].m_v2)?(.5f):(0));
              vx += ((nodes[ii][jj].m_v3)?(-.5f):(0));
              vx += ((nodes[ii][jj].m_v4)?(-1):(0));
              vx += ((nodes[ii][jj].m_v5)?(-.5f):(0));
              vx += ((nodes[ii][jj].m_v6)?(.5f):(0));
...
//     \===============  \  \ \\\\\\ \ =========== /  /                       //
  ...
  public void tick( int num_ticks) {
...
public void keyPressed() {
  ...
  if(key==CODED) {
    ...
    switch( key)
    {
      case ' ': g_dx*=1.1f; g_dy*=1.1f;
        //println("Zoom in");
        break;
      case 'b': g_dx/=1.1f; g_dy/=1.1f;
        //println("Zoom out");
        break;
...
// vim: set ft=cpp : syn on
  static public void main(String args[]) {
    PApplet.main(new String[] { "--bgcolor=#F0F0F0", "LGA" });
  }
}
